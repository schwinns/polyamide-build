...
for atom in atoms:
    ...
    mol_id += PA_mols # add the PA membrane
    ...
    if a_type == '15' and not atoms[atom]['delete']: # save waters where oxygen is within PA membrane cross section
        ...
        for a in atoms[atom]['bonded']: # save hydrogens bonded to good oxygens
            ...
            H_z = H_xyz[2] - zlo - PA_zlo - (zhi-zlo)

            distance = np.abs(z_shift - H_z) # distance in z direction
            
            if distance > (zhi-zlo)/2: # if bond is across water box
                if z_shift > (zhi-zlo)/2 - zlo - PA_zlo - (zhi-zlo): # if the oxygen is on top of water box
...
print('\n%d waters were added to the bottom reservoir' %(n_waters) )
if n_waters != n_bott/3:
    print('You probably lost a hydrogen... Exiting.')
    exit()
...
for bond in bonds: # write water bonds with new atom numbers for BOTTOM RESERVOIR
...
for angle in angles: # write water angles with new atom numbers for BOTTOM RESERVOIR
