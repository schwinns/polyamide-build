...
# LAMMPS
lmps = 'mpirun -n 64 lmp -sf omp -pk omp 1'
...
def polym_loop():
    ...
    while (bonds < bonds_tot):
        ...
        print '\nPolymerization for Bond %d took %s time' %(bonds, elapsed_time)
        if (code == 1):
            break
        em()
        em_time = time()
        elapsed_time = em_time - step_time
        print 'EM for Bond %d took %s time' %(bonds, elapsed_time)
        ...
        if (bonds % bonds_cyc == 0):
            ...
            if ((bonds/bonds_cyc) % md_cyc == 0):
                ...
                md(1)
        md_time = time()
        elapsed_time = md_time - em_time
        print 'MD for Bond %d took %s time' %(bonds, elapsed_time)
